...
function publishHtml(done, for_production = false) {
	let pipeline = gulp.src("src/**/*.{html,htm}").pipe(cache("html"));
...
function publishStatic(done) {
	return gulp
		.src(["src/**/*.*", "!src/**/*.{html,htm,css,scss,js,jsx}"])
		.pipe(cache("static"))
		.pipe(gulp.dest("dist"));
}
...
function publishCss(done, for_production = false) {
	...
	let pipeline = gulp
		.src(Object.values(entries.css), { base: "." })
		.pipe(
...
function watchFiles(done) {
	gulp.watch(
		["src/**/*.*", "!src/**/*.{html,htm,css,scss,js,jsx}"],
		gulp.series(publishStatic, reload)
	);
...
exports.build_dev = gulp.series(
	...
	publishStatic,
	publishCssDevelopment,
	publishJsDevelopment
);
